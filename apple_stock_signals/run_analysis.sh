...
case "$1" in
    "top5")
        ...
        python3 master_pipeline.py --list top_5
        ;;
    "top10")
        echo -e "${GREEN}Switching to Top 10 stocks...${NC}"
        python3 master_pipeline.py --list top_10
        ;;
    "top50")
        echo -e "${GREEN}Switching to Top 50 stocks...${NC}"
        python3 master_pipeline.py --list top_50
        ;;
    "tech")
        echo -e "${GREEN}Switching to Tech sector stocks...${NC}"
        python3 master_pipeline.py --list tech_sector
        ;;
    "health")
        echo -e "${GREEN}Switching to Healthcare sector stocks...${NC}"
        python3 master_pipeline.py --list healthcare_sector
        ;;
    "custom")
        echo -e "${GREEN}Switching to Custom stock list...${NC}"
        python3 master_pipeline.py --list custom
        ;;
    "add")
        if [ -z "$2" ]; then
        ...
        python3 master_pipeline.py --add-stock "$2"
        ;;
    "remove")
        if [ -z "$2" ]; then
        ...
        python3 master_pipeline.py --remove-stock "$2"
        ;;
    "config")
        echo -e "${BLUE}Current Configuration:${NC}"
        python3 master_pipeline.py --show-config
        ;;
    "force")
        echo -e "${YELLOW}Force running complete pipeline...${NC}"
        python3 master_pipeline.py --force
        ;;
    "quick")
...
if [ $? -eq 0 ]; then
    ...
    if [ -f "outputs/enhanced_analysis.json" ]; then
        ...
        python3 -c "
import json
with open('outputs/enhanced_analysis.json', 'r') as f:
    data = json.load(f)
    if 'results' in data:
        signals = {}
    ...
    echo "  • backtest_results/ - Backtesting data"
    echo "  • reports/         - Comprehensive reports"
    echo "  • historical_data/ - Stock data cache"
else
